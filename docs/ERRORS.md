...
Program: `sss-caps` (localnet ID generated at deploy)

| Code | Name | Message | Thrown When | Remediation |
|------|------|---------|-------------|-------------|
| 6000 | `Unauthorized` | Unauthorized - caller is not the caps authority | `update_caps_config` signer is not `caps_config.authority` | Use the keypair designated as caps authority. |
| 6001 | `SameCaps` | New caps are identical to current caps — no change needed | Both `global_cap` and `per_minter_cap` are unchanged | Provide at least one different cap value. |

### Cap Enforcement Errors (via SSS main program)

When a `CapsConfig` is passed in `remaining_accounts` during `mint_tokens`, the main SSS program reads it and may return:

| Code | Name | Thrown When |
|------|------|-------------|
| `6019` (SSS) | `SupplyCapExceeded` | The mint would exceed `caps_config.global_cap` |
| `6003` (SSS) | `QuotaExceeded` | The mint would exceed `caps_config.per_minter_cap` |

---

## SSS-Allowlist Module

Program: `sss-allowlist` (localnet ID generated at deploy)

| Code | Name | Message | Thrown When | Remediation |
|------|------|---------|-------------|-------------|
| 6000 | `Unauthorized` | Unauthorized - caller is not the allowlist authority | `add_to_allowlist_entry` or `remove_from_allowlist_entry` signer is not `allowlist_config.authority` | Use the keypair designated as allowlist authority. |
| 6001 | `AlreadyListed` | Address is already listed in the allowlist | `add_to_allowlist_entry` called for an address that already has an `AllowlistEntry` PDA | The address is already approved. Check before adding. |
| 6002 | `NotListed` | Address is not present in the allowlist | `remove_from_allowlist_entry` called for an address with no `AllowlistEntry` PDA | The address was never listed, or was already removed. |
| 6003 | `LabelTooLong` | Label exceeds the maximum length of 32 characters | `label.len() > 32` | Shorten the label. |
| 6004 | `SameMode` | New mode is identical to the current mode — no change needed | `update_allowlist_mode` called with the same mode as currently set | The allowlist mode is already what you want. |

---

## SSS-Timelock Module

Program: `sss-timelock` (localnet ID generated at deploy)

The timelock module enforces a mandatory delay between scheduling and executing governance operations.

| Code | Name | Message | Thrown When | Remediation |
|------|------|---------|-------------|-------------|
| 6000 | `Unauthorized` | Unauthorized - caller is not the timelock authority | Instruction signer is not `timelock_config.authority` | Use the timelock authority keypair. |
| 6001 | `DelayNotElapsed` | Timelock delay has not elapsed | `execute_op` called before `pending_op.scheduled_at + delay_seconds` | Wait the required delay period before executing. |
| 6002 | `AlreadyExecuted` | Operation has already been executed | `execute_op` called on an operation with `executed = true` | Each operation can only be executed once. |
| 6003 | `OperationCancelled` | Operation has been cancelled | `execute_op` called on a cancelled operation | Create a new `PendingOp` if the operation should be retried. |
| 6004 | `InvalidDelay` | Delay must be greater than zero | `initialize_timelock_config` called with `delay_seconds = 0` | Use a positive delay. Typical values: 86400 (1 day) to 604800 (1 week). |

---

## SSS-10 Async Mint/Redeem Module

Program: `sss-10` (localnet ID generated at deploy)

| Code | Name | Message | Thrown When | Remediation |
|------|------|---------|-------------|-------------|
| 6000 | `Unauthorized` | Unauthorized - caller is not the async config authority | `approve_mint_request`, `reject_mint_request`, etc. called by a non-authority signer | Use the keypair set as `async_config.authority`. |
| 6001 | `MemoTooLong` | Memo exceeds maximum length of 128 characters | `memo.len() > 128` in `submit_mint_request` or `submit_redeem_request` | Shorten the memo to 128 bytes or fewer. |
| 6002 | `InvalidStatus` | Request is not in the required status for this operation | Lifecycle transition attempted on a request in the wrong state (e.g., executing `Rejected` request) | Check the request's current `status` field before calling the instruction. Valid transitions: Pending→Approved/Rejected, Approved→Executed, Pending→Cancelled. |
| 6003 | `RequestNotFound` | Request account does not match the expected request_id | The PDA passed does not match the expected `request_id` seed | Derive the request PDA correctly using `["mint_request", async_config, request_id_le_bytes]`. |
| 6004 | `ZeroAmount` | Amount must be greater than zero | `amount = 0` in a request submission | Provide a positive amount. |
| 6005 | `AlreadyExecuted` | Request has already been executed | `execute_mint_request` called on a request with `status = Executed` | This request was already processed. It is idempotent only with respect to status — it cannot be re-executed. |
| 6006 | `MathOverflow` | Arithmetic overflow | Checked arithmetic overflowed `u64` | Internal error; review the requested amount values. |

---

## SSS-11 Credit Module

Program: `sss-11` (localnet ID generated at deploy)

| Code | Name | Message | Thrown When | Remediation |
|------|------|---------|-------------|-------------|
| 6000 | `Unauthorized` | Unauthorized - caller is not the credit authority | Admin instruction called by non-authority | Use the credit config authority keypair. |
| 6001 | `InsufficientCollateral` | Collateral deposit insufficient to open position at minimum ratio | `open_position` called with collateral below the minimum required for the requested debt | Increase the collateral amount or decrease the debt amount. |
| 6002 | `CollateralRatioTooLow` | Operation would bring collateral ratio below minimum | `withdraw_collateral` or `borrow_more` would cause `health < min_collateral_ratio_bps` | Repay some debt before withdrawing collateral, or deposit more collateral. |
| 6003 | `PositionHealthy` | Position is above liquidation threshold — cannot liquidate | `liquidate` called on a position that is not undercollateralized | The position's health factor is above the liquidation threshold. Monitor oracle prices. |
| 6004 | `PositionUnhealthy` | Position is below liquidation threshold — cannot withdraw | `withdraw_collateral` called on a position below the liquidation threshold | Repay debt to restore health before withdrawing. |
| 6005 | `OracleStale` | Oracle price is stale — cannot proceed | The credit module's oracle price is too old | Crank the oracle feed via `refresh_price` on the Oracle program. |
| 6006 | `ZeroAmount` | Amount must be greater than zero | Zero amount passed to deposit, withdraw, borrow, repay, or liquidate | Provide a positive amount. |
| 6007 | `MathOverflow` | Arithmetic overflow | Collateral ratio calculation overflowed | Internal error; review the collateral and debt amounts. |
| 6008 | `InvalidRatioParams` | Liquidation threshold must be below minimum collateral ratio | `initialize_credit_config` called with `liquidation_threshold_bps >= min_collateral_ratio_bps` | Set `liquidation_threshold_bps < min_collateral_ratio_bps`. Typical: 13000 threshold, 15000 minimum. |
